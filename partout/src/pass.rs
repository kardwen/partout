...
pub async fn fetch_entry(pass_id: String) -> (String, String) {
    ...
    PasswordStore::empty(event_tx).fetch_pass_details(pass_id.clone());
    loop {
...
pub async fn fetch_otp(pass_id: String) -> (String, String) {
    let (event_tx, event_rx) = mpsc::channel();
    PasswordStore::empty(event_tx).fetch_one_time_password(pass_id.clone());
    loop {
        let event = event_rx.recv().ok();
        match event {
...
pub async fn copy_id(pass_id: String) -> bool {
    ...
    PasswordStore::empty(event_tx).copy_pass_id(pass_id.clone());
...
pub async fn copy_password(pass_id: String) -> bool {
    let (event_tx, _event_rx) = mpsc::channel();
    PasswordStore::empty(event_tx).copy_password(pass_id.clone());
    sleep(Duration::from_secs(5));
    true
}
...
pub async fn copy_login(pass_id: String) -> bool {
    let (event_tx, _event_rx) = mpsc::channel();
    PasswordStore::empty(event_tx).copy_login(pass_id.clone());
    sleep(Duration::from_secs(5));
    true
}
...
pub async fn copy_otp(pass_id: String) -> bool {
    let (event_tx, _event_rx) = mpsc::channel();
    PasswordStore::empty(event_tx).copy_one_time_password(pass_id.clone());
    sleep(Duration::from_secs(5));
    true
}
